...
class VideoConv(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.mix_lstm = nn.LSTM(input_size=(self.v_att_out_dim + self.s_lstm_hidden_dim),
                                hidden_size=self.mix_lstm_hidden_dim, bidirectional=True)
...
class ActivityNet(Dataset):
    def __init__(self, v_h5, glove_file, mark_json, norm_time_steps=100, v_dim=500, s_dim=300):
        super().__init__()
        self.s_dim = s_dim
        self.v_dim = v_dim
        self.norm_time_steps = norm_time_steps
        self.c3d_h5 = h5py.File(v_h5, 'r', driver='core')
        self.mark = json.load(open(mark_json, 'r'))
        self.data_list = []
        print('constructing data list')
        for vid in self.mark.keys():
            duration = self.mark[vid]['duration']
            sentences = self.mark[vid]['sentences']
            timestamps = self.mark[vid]['timestamps']
            for s, t in zip(sentences, timestamps):
                self.data_list.append((vid, s, t, duration))
    ...
    def __getitem__(self, index: int):
        vid, sentence, timestamp, duration = self.data_list[index]
        vf_src = np.array(self.c3d_h5[vid]['c3d_features'])
        ...
        st = timestamp[0]
        et = timestamp[1]
        start_index = int(st / duration * self.norm_time_steps)
        end_index = int(et / duration * self.norm_time_steps)
        if start_index >= len(vf):
            start_index = len(vf) - 1
        if end_index >= len(vf):
            end_index = len(vf) - 1
        info = {'start_time': st, 'end_time': et, 'sentence': sentence, 'vname': vid, 'v_duration': duration,
...
def norm_batch(data, norm_time_steps=100):
    ...
    # return torch.from_numpy(np.array(empty_v)).float().cuda(), \
    #        torch.from_numpy(np.array(empty_s)).float().cuda(), \
    #        torch.LongTensor(vlen_batch).cuda(), \
    #        torch.LongTensor(slen_batch).cuda(), \
    #        torch.from_numpy(yt).float().cuda(), \
    #        torch.from_numpy(set).float().cuda(), \
    #        info_batch


def training_on_Activity(model: VideoConv, opt, data_loader, ):
...
def testing_on_Activity(model, data_loader, log='log_main86-act.txt'):
    ious = []
    preds = defaultdict(dict)
    f = open(log, 'a+')
    for batch_i, v in enumerate(data_loader):
        ...
        print(len(iou_batch))
        for i in range(len(sep)):
            iou = get_iou((info_batch[i]['start_time'], info_batch[i]['end_time']),
                          sep[i] / model.time_steps * info_batch[i]['v_duration'])
...
def run_on_Activity(train_epochs, eval_per_epochs, save_path):
    import json
    gm = GPUManager()
    ...
    train_data_loader = DataLoader(
        ActivityNet(r"data/ActivityNetCaptions/activitynet_v1-3/sub_activitynet_v1-3.c3d.hdf5",
                    'glove_wiki/glove.6B.300d.txt',
                    'data/ActivityNetCaptions/train.json'),
        batch_size=32, shuffle=False, collate_fn=norm_batch, num_workers=4)

    test_data_loader = DataLoader(
        ActivityNet(r"data/ActivityNetCaptions/activitynet_v1-3/sub_activitynet_v1-3.c3d.hdf5",
                    'glove_wiki/glove.6B.300d.txt',
                    'data/ActivityNetCaptions/val_1.json'),
        batch_size=128, shuffle=False, collate_fn=norm_batch, num_workers=4)

    for epoch_index in range(train_epochs):
        model.train(True)
        loss = training_on_Activity(model, opt, train_data_loader)
        ...
        if epoch_index % eval_per_epochs == 0:
            ...
            testing_on_Activity(model, test_data_loader)
...
if __name__ == '__main__':
    ...
    while (not success):
        try:
            run_on_Activity(100, 1, 'resources/main35_')
            success = True
